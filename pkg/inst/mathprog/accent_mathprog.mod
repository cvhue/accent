...
table tab_patients IN "CSV" "../examples/patients.csv" :
    J <- [patient];
    
table tab_therapists IN "CSV" "../examples/therapists.csv" :
    I <- [therapist], tmp ~ skill;

table tab_patientskills IN "CSV" "../examples/patientskills.csv" :
  PS <- [patient,skill];

table tab_preferences IN "CSV" "../examples/parameters.csv" :
